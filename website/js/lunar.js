/**
 * 农历换算工具（标准万年历数据，覆盖 1900–2100）
 * 同时兼容小程序 require 与浏览器 <script> 两种引入方式。
 */
const lunarInfo = [
...
const gzTian = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
const gzDi = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
 ...
 *   monthCn:string,dayCn:string,gzYear:string,animal:string}}
 */
function solarToLunar(date) {
  ...
  const month = i;
  const day = offset + 1;
  const gzIndex = (year - 4) % 60;
  return {
    year, month, day, isLeap,
    monthCn: (isLeap ? '闰' : '') + monthCn[month - 1] + '月',
    dayCn: dayCn[day - 1],
    gzYear: gzTian[gzIndex % 10] + gzDi[gzIndex % 12],
    animal: animals[gzIndex % 12]
  };
}

/** 今日信息（给首页/网站首屏用） */
function getTodayInfo(now) {
  const d = now || new Date();
  const l = solarToLunar(d);
  return {
    solarText: d.getFullYear() + ' 年 ' + (d.getMonth() + 1) + ' 月 ' + d.getDate() + ' 日',
    weekText: '星期' + weekCn[d.getDay()],
    lunarText: l.monthCn + l.dayCn,
    gzText: l.gzYear + '年 · 生肖' + l.animal,
    lunar: l
  };
}

const Lunar = { solarToLunar, getTodayInfo };

// 浏览器环境挂到 window；小程序/Node 走 module.exports
if (typeof window !== 'undefined') { window.Lunar = Lunar; }
