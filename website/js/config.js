/**
 * 网站全局配置 —— 与小程序 config/donation.js 保持一致
 * mode: 'redirect'（跳转官方公益平台，不碰钱） | 'self'（未来自有收款，见 docs/收款接口契约.md）
 */
const SITE_CONFIG = {
  name: '静心日课',
  donation: {
    mode: 'redirect',
    redirect: {
      tip: '本站不代收善款。随喜请前往官方公益平台，善款直达项目方对公账户。',
      channels: [
        {
          id: 'gongyi',
          name: '腾讯公益',
          desc: '民政部首批指定互联网募捐平台，可开捐赠证书与票据',
          // TODO: 上线时替换为你想支持的具体项目页链接
          link: 'https://gongyi.qq.com/'
        }
      ]
    },
    self: {
      tip: '随喜将用于经书印制、场地维护与线上服务的持续运行。',
      apiBase: '', // TODO: 后端就绪后填入，如 https://api.example.com
      presetAmounts: [1, 6, 10, 66, 88, 366],
      createOrder: '/api/donation/orders'
    }
  },
  quotes: [
    '诸恶莫作，众善奉行，自净其意，是诸佛教。 ——《法句经》',
    '应无所住，而生其心。 ——《金刚经》',
    '心无挂碍，无有恐怖，远离颠倒梦想，究竟涅槃。 ——《心经》',
    '一切有为法，如梦幻泡影，如露亦如电，应作如是观。 ——《金刚经》',
    '制心一处，无事不办。 ——《佛遗教经》',
    '若人欲了知，三世一切佛，应观法界性，一切唯心造。 ——《华严经》',
    '菩萨畏因，众生畏果。 ——《涅槃经》',
    '是日已过，命亦随减，如少水鱼，斯有何乐。 ——《法句经·无常品》',
    '人身难得，佛法难闻；此身不向今生度，更向何生度此身。',
    '迷时师度，悟了自度。 ——《六祖坛经》',
    '菩提本无树，明镜亦非台，本来无一物，何处惹尘埃。 ——《六祖坛经》',
    '勤修戒定慧，息灭贪嗔痴。',
    '知足之法，即是富乐安隐之处。 ——《佛遗教经》',
    '上报四重恩，下济三途苦。',
    '于一毫端现宝王刹，坐微尘里转大法轮。 ——《楞严经》',
    '平等饶益一切众生。 ——《普贤行愿品》',
    '于诸众生，视若自己。 ——《无量寿经》',
    '一花一世界，一叶一如来。 ——《华严经》',
    '不为自身求安乐，但愿众生得离苦。 ——《华严经》',
    '放下著。 ——《五灯会元》'
  ]
};

window.SITE_CONFIG = SITE_CONFIG;
